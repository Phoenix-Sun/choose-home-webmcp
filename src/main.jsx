...
import { DISTRICT_OPTIONS_BY_CITY, getDistrictOptions, isDistrictZipShared } from './domain/districts.js'
...
const CONSTRAINT_SCHEMA_PROPERTIES = {
  ...
  district: { type: 'string', description: '行政區名稱；必須同時提供所屬 city，重名行政區不可只靠區名判斷。' },
...
function criteriaToManualDraft(criteria = DEFAULT_CRITERIA) {
  ...
  return {
    ...
    cities,
    districtKey: criteria.district ? `${criteria.city}::${criteria.district}` : '',
...
function App() {
  ...
  const [collaborationGuideOpen, setCollaborationGuideOpen] = useState(false)
  ...
  function applyManualCriteria(event) {
    ...
    const maxPrice = manualDraft.maxPrice === '' ? null : Number(manualDraft.maxPrice)
    if (maxPrice != null && (!Number.isFinite(maxPrice) || maxPrice <= 0)) {
      showNotice('總價上限必須大於 0')
      return
    }
    const selectedCities = manualDraft.cityGroup === '雙北市'
      ? ['台北市', '新北市']
      : manualDraft.cityGroup === MULTI_CITY_VALUE
        ? manualDraft.cities
        : [manualDraft.cityGroup]
    const [districtCity = '', district = ''] = manualDraft.districtKey.split('::')
    const cities = district && selectedCities.includes(districtCity) ? [districtCity] : selectedCities
    const next = {
      ...criteria,
      city: cities[0],
      cities,
      district: district && cities.includes(districtCity) ? district : '',
      maxPrice,
  ...
  return (
    <div className={`app-shell ${compareIds.length ? 'has-compare-tray' : ''}`}>
      <header className="topbar"><div className="brand-lockup"><span className="brand-mark"><span></span><span></span><span></span></span><span className="brand-name">選好宅</span><span className="brand-slash">｜</span><span className="brand-cn">找房比較助手</span><button type="button" className={`ai-guide-trigger ${aiMode ? 'is-connected' : ''}`} aria-label={aiMode ? 'ChatGPT 正在協作，查看使用說明' : '查看 ChatGPT 協作使用說明'} onClick={() => setCollaborationGuideOpen(true)}><span className="ai-guide-dot"></span>{aiMode ? 'ChatGPT 協作中' : 'ChatGPT 協作'}</button></div><div className="topbar-actions"><div className={`source-status ${sourceState.status}`} aria-label={sourceLabel}><span className="status-dot"></span><span>{sourceLabel}</span></div><button className="quiet-button restart-button" aria-label="重新開始" onClick={() => setRestartOpen(true)} disabled={isResetting}><span>重新開始</span></button><button className="quiet-button" onClick={() => setDecisionCard(true)} disabled={!selected || selected.outsideCurrentSearch}><Icon name="share" size={17} /><span>分享</span></button></div></header>
      <main>
      ...
      {collaborationGuideOpen ? <CollaborationGuideModal aiMode={aiMode} onClose={() => setCollaborationGuideOpen(false)} /> : null}
...
function ManualSearchBar({ draft, onChange, onSubmit, loading, hasActiveSearch, embedded = false }) {
  ...
  const selectedCities = draft.cityGroup === '雙北市'
    ? ['台北市', '新北市']
    : draft.cityGroup === MULTI_CITY_VALUE
      ? draft.cities || []
      : [draft.cityGroup]
  const districtOptions = getDistrictOptions(selectedCities)
  const updateCity = (event) => {
    const cityGroup = event.target.value
    const cities = cityGroup === '雙北市' ? ['台北市', '新北市'] : cityGroup === MULTI_CITY_VALUE ? draft.cities || [] : [cityGroup]
    onChange((current) => ({ ...current, cityGroup, cityGroupLabel: formatCitySelection(cities, cityGroup), cities, districtKey: '' }))
  }
  return <form className={`manual-search-bar ${embedded ? 'embedded' : ''}`} onSubmit={onSubmit} aria-label="自行設定找房條件">
    <label><span>縣市</span><select value={draft.cityGroup} onChange={updateCity}>
    ...
    <label><span>行政區</span><select value={draft.districtKey} onChange={update('districtKey')}>
      <option value="">{selectedCities.length === 1 ? `全${selectedCities[0]}` : '全部地區'}</option>
      {selectedCities.length > 1
        ? selectedCities.map((city) => <optgroup key={city} label={city}>{(DISTRICT_OPTIONS_BY_CITY[city] || []).map(([name]) => <option key={`${city}-${name}`} value={`${city}::${name}`}>{name}</option>)}</optgroup>)
        : districtOptions.map(({ city, name }) => <option key={`${city}-${name}`} value={`${city}::${name}`}>{name}</option>)}
    </select></label>
    <label><span>總價上限</span><span className="field-with-unit"><input type="number" min="1" placeholder="不限" value={draft.maxPrice} onChange={update('maxPrice')} /><small>萬</small></span></label>
...
function EvidencePanel({ candidates, recommendations, selected, selectedId, pinnedIds, compareIds, onSelect, onPin, onCompare, onOpenFavorites, activity, sourceState, criteria, onEnrich, isEnriching, resultView, onResultView, onLoadMore, onLoadAll, onCancelLoadAll, isLoadingMore, isLoadingAll, hasActiveSearch }) {
  ...
  const requiresExactDistrictPostFilter = sourceState.requiresExactDistrictPostFilter ?? isDistrictZipShared(criteria.city, criteria.district)
  const sourceResultText = requiresExactDistrictPostFilter
    ? `已檢查 ${sourceState.inspectedCount || 0} / ${sourceState.totalCount || 0} 筆來源資料，找到 ${sourceState.returnedCount ?? 0} 間${criteria.district}物件`
    : `公開來源共有 ${sourceState.totalCount || 0} 筆相關物件，目前顯示 ${sourceState.returnedCount ?? 0} 間`
...
function CollaborationGuideModal({ aiMode, onClose }) {
  const dialogRef = useDialogFocus()
  return <div className="modal-backdrop" onClick={onClose}><section ref={dialogRef} tabIndex="-1" className="workspace-modal collaboration-guide-modal" role="dialog" aria-modal="true" aria-label="ChatGPT 協作使用說明" onClick={(event) => event.stopPropagation()}><div className="workspace-modal-head"><div><h2>用 ChatGPT 一起找房</h2><p>你可以自行操作，也可以讓 ChatGPT 與你共用目前的條件、地圖、收藏與比較。</p></div><button className="modal-close" onClick={onClose} aria-label="關閉"><Icon name="close" /></button></div><div className="collaboration-guide-body"><div className="collaboration-modes"><article><span>自己操作網站</span><strong>篩選、看地圖、收藏與比較都能直接使用</strong><p>不需要透過 AI，也能完成一般找房流程。</p></article><article className="agent-mode"><span>透過 ChatGPT 協作</span><strong>用自然語言處理多條件與反覆取捨</strong><p>適合條件很多、想比較不同方案，或需要解釋變化時。</p></article></div><div className="collaboration-capabilities"><h3>ChatGPT 可以幫你</h3><article><span>1</span><div><strong>一次理解多個找房條件</strong><p>例如地區、預算、房數、屋齡與捷運距離，不必逐欄操作。</p></div></article><article><span>2</span><div><strong>邊討論，邊同步調整結果</strong><p>先看條件變更會帶來什麼影響；已收藏的物件不會被悄悄移除。</p></div></article><article><span>3</span><div><strong>讀取完整清單並說明取捨</strong><p>繼續載入公開物件、整理比較事實，也會明確指出尚待確認的資料。</p></div></article></div><div className="collaboration-start"><div><span>怎麼開始</span><strong>在支援網站協作的 ChatGPT 中開啟本頁，直接說出你的需求。</strong></div><blockquote>「請用這個網站找雙北 2,500 萬內、屋齡不要太老、近捷運的住宅，房數不限。」</blockquote><small>如果目前環境不支援 ChatGPT 協作，網站上的所有找房功能仍可自行使用。</small></div></div><div className="collaboration-guide-foot"><span className={aiMode ? 'connected' : ''}><i></i>{aiMode ? '目前已進入 ChatGPT 協作模式' : '目前是一般網站模式'}</span><button type="button" onClick={onClose}>開始使用</button></div></section></div>
}
