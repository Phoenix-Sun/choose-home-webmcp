// 中華郵政「3碼郵遞區號與行政區中心點經緯度對照表」的行政區欄位。
// 釣魚台列嶼、東沙群島、南沙群島不屬於本網站的住宅搜尋範圍。
...
export const DISTRICT_NAMES = [...new Set(
  Object.values(DISTRICT_OPTIONS_BY_CITY).flatMap((items) => items.map(([name]) => name)),
)].sort((left, right) => right.length - left.length)

export function getDistrictOptions(cities = []) {
  return cities.flatMap((city) => (DISTRICT_OPTIONS_BY_CITY[city] || []).map(([name, zipCode]) => ({ city, name, zipCode })))
}

export function getDistrictZipCode(city, district) {
  return DISTRICT_OPTIONS_BY_CITY[city]?.find(([name]) => name === district)?.[1] || ''
}

export function getDistrictOwners(district) {
  return Object.keys(DISTRICT_OPTIONS_BY_CITY).filter((city) => getDistrictZipCode(city, district))
}

export function isDistrictZipShared(city, district) {
  const zipCode = getDistrictZipCode(city, district)
  return Boolean(zipCode) && (DISTRICT_OPTIONS_BY_CITY[city] || []).filter(([, zip]) => zip === zipCode).length > 1
}
